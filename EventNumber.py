...
tight=pandas.read_csv('cvs/test_tight'+'.csv')
tight.name='tight'
loose=pandas.read_csv('cvs/test_loose'+'.csv')
loose.name='loose'
...
#print(len(tight))
#tight=tight[tight['FakeMIsolationFCTight']==1] #Iso WP
#tight=tight[abs(tight['FakeMd0sig'])<=3] #Iso WP
#tight=tight[tight['FakeMQualityTight']==1] #Iso WP
#print(len(tight))
...
'''
tight1=tight['eventNumber'].to_numpy()
loose1=loose['eventNumber'].to_numpy()

check=all(item in loose1 for item in tight1) #are all items in tight in loose?
print(check)

test=set(tight1).issubset(loose1)
print(test)
'''
