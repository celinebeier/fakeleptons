...
outputFile=ROOT.TFile('/afs/cern.ch/user/c/cbeier/fakeleptons/root/'+"Comp_VR.root","recreate")
...
for name in name_list:
    if name=="error":
        ...
        MC_loose_16=pandas.read_csv('/eos/user/c/cbeier/MC16/'+name+'_loose.csv')
        MC_loose_16=MC_loose_16[(MC_loose_16['E1Pt']>=27)]
        MC_loose_16=MC_loose_16[(MC_loose_16['VRSelection']==1)]
        MC_loose_17=pandas.read_csv('/eos/user/c/cbeier/MC17/'+name+'_loose.csv')
        MC_loose_17=MC_loose_17[(MC_loose_17['E1Pt']>=27)]
        MC_loose_17=MC_loose_17[(MC_loose_17['VRSelection']==1)]
        MC_loose_18=pandas.read_csv('/eos/user/c/cbeier/MC18/'+name+'_loose.csv')
        MC_loose_18=MC_loose_18[(MC_loose_18['E1Pt']>=27)]
        MC_loose_18=MC_loose_18[(MC_loose_18['VRSelection']==1)]
    
        #MC_loose=MC_loose_18
    
        MC_loose=pandas.concat([MC_loose_16, MC_loose_17, MC_loose_18])
    
        #print(max(MC_loose['weight']))
        #print(len(MC_loose))
        #print(MC_loose)
        #MC_loose=MC_loose[(MC_loose['weight']<5)]

        #MC_loose=MC_loose[(MC_loose['VRSelection']==1)]
    ...
    #xticks_labels=['Unknown','KnownUnknown','Prompt electrons','Charge-flip electrons','Prompt muons','Prompt photon-conversions','Electrons from muons','Tau decays','b-hadron decays','c-hadron decays','Light-flavour decays','Charge-flip muons']
        ...
        n_error,bins,patches=ax.hist(MC_tight['E1ID'],bins=bins,align='left',weights=(MC_tight['weight'])**2) #weights=MC_tight['weight']
    ...
    #print(n[8],n[9],n[10])
        ...
        hist.Write()
'''    
error_0=np.sqrt(sum(x**2 for x in error_0 if x > 0))    
error_1=np.sqrt(sum(x**2 for x in error_1 if x > 0))    
error_2=np.sqrt(sum(x**2 for x in error_2 if x > 0))    
error_3=np.sqrt(sum(x**2 for x in error_3 if x > 0))    
error_4=np.sqrt(sum(x**2 for x in error_4 if x > 0))    
error_5=np.sqrt(sum(x**2 for x in error_5 if x > 0))    
error_6=np.sqrt(sum(x**2 for x in error_6 if x > 0))    
error_7=np.sqrt(sum(x**2 for x in error_7 if x > 0))    
error_8=np.sqrt(sum(x**2 for x in error_8 if x > 0))    
error_9=np.sqrt(sum(x**2 for x in error_9 if x > 0))    
error_10=np.sqrt(sum(x**2 for x in error_10 if x > 0))    
error_11=np.sqrt(sum(x**2 for x in error_11 if x > 0))
