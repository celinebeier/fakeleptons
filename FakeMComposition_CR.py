...
outputFile=ROOT.TFile('/afs/cern.ch/user/c/cbeier/fakeleptons/root_FakeM/'+"Comp_CR.root","recreate")
...
for name in name_list:
    if name=="Zjets":
        ...
        MC_loose=MC_loose[(MC_loose['FakeMPt']>=27)]
        ...
        plt.figure(1)
        fig,ax=plt.subplots()
        n,bins,patches=ax.hist(MC_tight['FakeMID'],bins=bins,align='left',weights=MC_tight['weight']) #weights=MC_tight['weight']
        n_error,bins,patches=ax.hist(MC_tight['FakeMID'],bins=bins,align='left',weights=(MC_tight['weight'])**2) #weights=MC_tight['weight']
