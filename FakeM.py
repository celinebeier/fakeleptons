...
data_loose=pandas.read_csv('/eos/user/c/cbeier/csv_FakeM/'+name+'_Data_loose.csv')
print('reading in MC')
WZ_loose=pandas.read_csv('/eos/user/c/cbeier/csv_FakeM/'+name+'_MC_loose.csv')
print('start',len(data_loose),len(WZ_loose),sum(WZ_loose['weight']))

#print('PT MAX:',data_loose['E1Pt'].max(),WZ_loose['E1Pt'].max())
#print('ETA MAX:',data_loose['E1Eta'].max(),WZ_loose['E1Eta'].max())

#data_loose=data_loose[(data_loose['VRSelection']==1)]
#WZ_loose=WZ_loose[(WZ_loose['VRSelection']==1)]
#print('region',len(data_loose),len(WZ_loose),sum(WZ_loose['weight']))

data_loose=data_loose[(data_loose['invmWy_fit']<=max(bins))]
WZ_loose=WZ_loose[(WZ_loose['invmWy_fit']<=max(bins))]
print('max bin',len(data_loose),len(WZ_loose),sum(WZ_loose['weight']))

#choose fake e's
data_loose=data_loose[(data_loose['M27Counter']==1)]
WZ_loose=WZ_loose[(WZ_loose['M27Counter']==1)]
print('fake E',len(data_loose),len(WZ_loose),sum(WZ_loose['weight']))

#data_test=data_loose[(data_loose['ElectronCounter']==2)]
#print(len(data_test))
#print(data_test)
#print(data_test['E1Pt'],data_test['E2Pt'],data_test['M1Pt'],data_test['M2Pt'])

#add PT cuts - 27GeV for analysis
data_loose=data_loose[(data_loose['M1Pt']>=27)]
WZ_loose=WZ_loose[(WZ_loose['M1Pt']>=27)]
print('Pt cut',len(data_loose),len(WZ_loose),sum(WZ_loose['weight']))

#loose cut
data_loose=data_loose[(data_loose['M1d0sig']<=3)]
WZ_loose=WZ_loose[(WZ_loose['M1d0sig']<=3)]
print('d0sig cut',len(data_loose),len(WZ_loose),sum(WZ_loose['weight']))

#eta -> abs(eta)
data_loose['M1Eta']=abs(data_loose['M1Eta'])
WZ_loose['M1Eta']=abs(WZ_loose['M1Eta'])

#truthmatching
WZ_loose=WZ_loose[(WZ_loose['M1ID']==4)]
...
data_nt=data_loose[(data_loose['M1IsolationFCTight']==0)&(data_loose['M1QualityTight']==0)]
WZ_nt=WZ_loose[(WZ_loose['M1IsolationFCTight']==0)&(WZ_loose['M1QualityTight']==0)]
...
#get fake factor
#fakefactor=np.genfromtxt("root_FakeM/"+'MET10Up/'+"fakefactor.csv", delimiter=',')
#errorfakefactor=np.genfromtxt("root_FakeM/"+'MET10Up/'+"errorfakefactor.csv", delimiter=',')
fakefactor=np.genfromtxt("root_FakeM/"+"fakefactor.csv", delimiter=',')
print(fakefactor)
#fakefactor=fakefactor*1.5
#print(fakefactor)
errorfakefactor=np.genfromtxt("root_FakeM/"+"errorfakefactor.csv", delimiter=',')
#print(fakefactor)

count=0

for i in range(0,len(data_loose)):
    if (data_loose.iloc[i]['M1IsolationFCTight'] == 0 and data_loose.iloc[i]['M1QualityTight'] == 0):
        count=count+1
        #print(count)
        ...
        plt.figure(1)
        hist,xbins,ybins,im=plt.hist2d(row['M1Pt'], row['M1Eta'], bins=edges)
        hist=hist.T
...
for i in range(0,len(WZ_loose)):
    if (WZ_loose.iloc[i]['M1IsolationFCTight'] == 0 and WZ_loose.iloc[i]['M1QualityTight'] == 0):
        #count=count+1
        #print(count)
        #print(data_loose.iloc[i]['E1Pt'],data_loose.iloc[i]['E1Eta'])
        row=WZ_loose.iloc[[i]]
...
#print('in bins:',len(data_loose[(data_loose['invmWy_fit']<500)]),len(WZ_loose[(WZ_loose['invmWy_fit']<500)]),sum(WZ_loose[(WZ_loose['invmWy_fit']<500)]['weight']))
data_plot=pandas.concat([data_loose,WZ_loose])
#print(sum(data_plot['F']),sum(data_plot['weight']),sum(data_plot['F']*data_plot['weight']))
...
error_str=str(error)
#with open('root_FakeM/'+'MET10Up/'+name+'_FakeE.txt', "w") as file:
with open('root_FakeM/'+name+'_FakeE.txt', "w") as file:
   lines = [name+'\n', 'total expexted fakes: '+expectedFakes_str+'\n', 'error: '+error_str+'\n']
   file.writelines(lines)
   file.close()

#outputFile=ROOT.TFile('/afs/cern.ch/user/c/cbeier/fakeleptons/root_FakeM/'+'MET10Up/'+name+"_FakeE.root","recreate")
outputFile=ROOT.TFile('/afs/cern.ch/user/c/cbeier/fakeleptons/root_FakeM/'+name+"_FakeE.root","recreate")
